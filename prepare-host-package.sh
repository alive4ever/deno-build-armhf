...
sudo mmdebstrap --arch=armhf --keyring="$RASPBIAN_KEYRING" --include sudo,curl,build-essential,devscripts,protobuf-compiler,python3,python3-venv,ninja-build,generate-ninja,cmake,git,nodejs,sccache trixie /var/lib/machines/armhf-raspbian http://raspbian.raspberrypi.com/raspbian
